...
### Fluent RSpec Output
RSpec chains descripitons, contexts, and specifications into a sentence to
communicate to the reader. Use this.
...
# RSpec will unroll the strings into a sentence. Make it be a valid, descriptive
# sentence that tells the story of the test:
# => "MyClass validations without a valid user is not valid"
...
# This unrolls into a mash of nouns:
# => "MyClass validations no user fails"
```


### Context Blocks
Start with a conjunction. "when," "with," or "without" and describe a condition
or a context.
...
## Tell The Whole Story In The Spec

In app code, DRY means Don't Repeat Yourself. Everything should be placed in a
single, convenient place. Programmers love using let() and let!() because these
cache and reuse the object being tested. But this HIDES the object being tested
far away from the test. Favor declaring your object

## Antipatterns

### Put Arrange steps in before() but never put Act steps in before

Arrange, Act, Assert: Act steps MUST live in the specification block.

The before block can contain expect() calls, as they are asserted at the end of
the spec block.

### Do not use expect() in before blocks

expect will arrange an assertion, so it should be good in the before block, but
this assertion is implicit. Favor explicit instance_doubles and use
expect(object).to have_received(:method) instead.

### Don't use allow, use expect().at_least(:once)

Expect will fail if a method isn't called the correct number of times. Allow
permits any number, including zero. If you delete the code or fail to call that
method, the spec is now dead code.

expect will fail early and loud, showing you the spec that needs to be cleaned
up.

For the same 1..n variance that allow provides, use expect() with
at_least(:once).

### Don't use subject or described_class

Write it out. You're not saving any time. You're making it harder to remember
what's being tested, and if you need to rename the subject it doesn't take
nearly as much time to just replace them all as you think.

Don't use the DSL when bare ruby is just as clean.

### Never use subject as a verb that causes side effects

I don't like subject because the DSL hides the information is something easy to
understand.

Using subject as a verb to trigger side effects hides the information in
something hard to understand, all so you can save some keystrokes. Don't do it.

Also it requires you to set your Asserts in the before block to even work. It's
literally more work to make your code harder to read.

```ruby
# ❌ Bad
subject { call_some_method }
before { expect(...) } # Assertion created in the Arrange step
...
### If the DSL cleans up messy code, THAT'S A DEFECT. DON'T USE IT.

If it's a pain in the neck to declare your subject, your code is a pain for your
coworkers to use. Don't use RSpec to make your crappy code easier to
tolerate. That's just taping a diaper over the mess.

Clean up your code. Your specs will clean up themselves.
